...
def visualise_intermediate(
    ...
    """
    Visualize intermediate graph states during processing.
    
    Args:
        visualise_data_dict (dict): Dictionary to store visualization data
        inds_to_visualise (list): List of indices to visualize
        graph: Input graph object with batch information
        mask: Boolean mask for selecting nodes
        subgraphs: Dictionary containing subgraph cluster information
        atom_indices: Indices of atoms for each graph
    ...
    Returns:
        dict: Updated visualization data dictionary
    """
...
def visualise_partitioning(
    G, cluster_labels_list, mv_fractional, fig, directed=False, hs_scaling=3
):
    """
    Visualize graph partitioning using graph-tool's block state visualization.
    
    Args:
        G: NetworkX graph to visualize
        cluster_labels_list: List of cluster labels for each node
        mv_fractional: Fractional values for vertex sizing
        fig: Matplotlib figure to draw on
        directed (bool): Whether the graph is directed
        hs_scaling (int): Scaling factor for halo sizes
    
    Returns:
        BlockState: The graph-tool block state object
    """
    ...
    # Initialize block assignments and halo sizes
    ...
    # Assign clusters and scale halo sizes
...
def visualisation_log(
    ...
    """
    Log visualization data to wandb for monitoring training progress.
    
    Args:
        visualise_data_dict: Dictionary containing visualization data
        inds_to_visualise: List of indices to visualize
        visualise_step: Current step for logging
        total_costs: Total cost values for each graph
        baseline: Baseline cost values
        cost_terms: Dictionary of individual cost terms
        x_a_fractional: Fractional assignment values
        partioning (bool): Whether to include partitioning visualization
        directed (bool): Whether graphs are directed
    ...
    if visualise_data_dict is None and subgraphs is not None:
        for ind_to_visualise in inds_to_visualise:
            ...
            # Generate partitioning visualization
            block_state = visualise_partitioning(
                G_vis, cluster_labels_list, mv_fractional, fig, directed=directed
            )
            ...
            # Log to wandb
            wandb.log(
                {"graph_partitioning_" + str(ind_to_visualise): wandb.Image(plt)},
                step=visualise_step,
            )
            plt.close()
    else:
        # Handle normal visualization case
        for ind_to_visualise in inds_to_visualise:
            if ind_to_visualise in visualise_data_dict:
                ...
                num_rows = (
                    int(math.ceil((len(visualise_data_list) + 1) / 2))
                    if partioning
                    else int(math.ceil(len(visualise_data_list) / 2))
                )
                ...
                if partioning:
                    ax_i = (
                        ax[len(visualise_data_list) // 2, len(visualise_data_list) % 2]
                        if num_rows >= 2
                    ...
                    if "dl" in visualise_data_list[0]:
                        ...
                        if cost_terms is None:
                            ax_i.title.set_text(
                                "num clusters: {:.4f}, baseline dl: {:.4f}, dl: {:.4f}".format(
                                    max(block_state.get_blocks()) + 1, baseline_dl, dl
                                )
                            )
                        else:
                        ...
                        ax_i.title.set_text(
                            "num clusters: {}".format(max(block_state.get_blocks()) + 1)
                        )
                        
            # Log to wandb
            wandb.log(
                {"graph_" + str(ind_to_visualise): wandb.Image(plt)},
                step=visualise_step,
            )
            plt.close()
    ...
    for i in range(init_i, final_i):
        ...
        if node_attr_dims is not None:
            ...
            for node in G.nodes(data=True):
                ...
                # Assign colors based on first attribute
                node_colors.append(
                    colormap(attr_dict["0"] / len(attr_mapping.node_attr_values[0]))
                )
        else:
            node_labels = None
