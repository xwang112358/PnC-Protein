...
def prepare_logs(args, path, fold_idx=""):
    ...
    if "model_name" in args:
        results_folder = os.path.join(
            path, "results", args["results_folder"], str(fold_idx), args["model_name"]
        )
    else:
        results_folder = os.path.join(
            path,
    ...
    **kwargs,
    ...
    with torch.no_grad():
        x_a, b_probs, delta_prob, atom_probs, cut_size_probs = (
            dictionary_probs_model.prune_universe(agent.train)
        )
        kwargs_test = {
            "x_a": x_a,  # map subgraphs only to the current dictionary
            "visualise": kwargs["visualise"] if "visualise" in kwargs else False,
            "inds_to_visualise": kwargs["inds_to_visualise"]
            if "inds_to_visualise" in kwargs
            else 0,
            "visualise_step": kwargs["visualise_step"]
            if "visualise_step" in kwargs
            else 0,
        }
        kwargs_heuristic = {
            "compute_dl": agent.env.compute_dl,
            "x_a": x_a,  # unused
            "b_probs": b_probs,
        ...
        for iteration in tqdm(range(n_iters)):
            try:
                ...
                data = next(data_iterator)
            data = data.to(device)
            kwargs_test["visualise"] = (
                kwargs_test["visualise"] if iteration == 0 else False
            )
            ...
            atoms_in_dict = (
                x_a[subgraphs["atom_indices"]][subgraphs["n_h"] != 0].sum().item()
            )
            empirical_subgraphs_types_freqs[0] += atoms_in_dict
            empirical_subgraphs_types_freqs[1] += (
                subgraphs["n_h"] != 0
            ).sum().item() - atoms_in_dict
    ...
    evaluation_metric_w_dict, evaluation_metric = evaluation_fn(
        compressed_size, uncompressed_size, dictionary_cost
    )
    atom_indices_flattened = torch.cat(
        [atom_indices_batch.flatten() for atom_indices_batch in atom_indices]
    )

    atom_indices_flattened = atom_indices_flattened[atom_indices_flattened != -1]
    if len(atom_indices_flattened) != 0:
        _, empirical_atom_probs = agent.env.estimate_atom_freqs_probs(
            atom_indices_flattened
        )
    else:
        empirical_atom_probs = torch.zeros_like(atom_probs)
    metrics_dict = {
        "compressed_size": compressed_size.sum().item(),
...
def test_fixed_part(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        compressed_size, uncompressed_size = ml_estimation_fixed_part(
            subgraphs, agent, x_a, b_probs, delta_prob, atom_probs, cut_size_probs
        )
        if kwargs_test["visualise"]:
            visualisation_log(
                visualise_data_dict,
...
def realtime_logger(
    ...
    **kwargs,
    ...
    for i, loader in enumerate(loaders):
        ...
        num_params = dictionary_probs_model.count_params(
            compression_metrics["x_a"][0 : len(agent.env.dictionary)]
        )
        parameter_cost = num_params * kwargs["bits_per_parameter"]
        bpe_dict = bits_per_edge(
            raw_bits_model[i],
            total_num_edges[i],
            compression_metrics["dictionary_cost"],
        )
        bpe_dict_param = bits_per_edge(
            raw_bits_model[i],
            total_num_edges[i],
        ...
        if wandb_realtime:
            if "empirical_subgraph_types_freqs" in compression_metrics:
                fig = plt.figure()
                plt.bar(
                    list(range(len(compression_metrics["empirical_subgraph_types"]))),
                    compression_metrics["empirical_subgraph_types"],
                )
                wandb.log(
                    {
                        "empirical subgraph types freqs {}".format(
                            dataset_name
                        ): wandb.Image(fig)
                    },
                    step=epoch,
            ...
            if "empirical_atom_probs" in compression_metrics:
                fig = plt.figure()
                plt.bar(
                    list(range(len(compression_metrics["empirical_atom_probs"]))),
                    compression_metrics["empirical_atom_probs"].tolist(),
                )
                wandb.log(
                    {
                        "empirical atom probabilities {}".format(
                            dataset_name
                        ): wandb.Image(fig)
                    },
                    step=epoch,
            ...
            if "empirical_b_freqs" in compression_metrics:
                fig = plt.figure()
                plt.bar(
                    list(range(len(compression_metrics["empirical_b_freqs"]))),
                    (
                        compression_metrics["empirical_b_freqs"]
                        / compression_metrics["empirical_b_freqs"].sum()
                    ).tolist(),
                )
                wandb.log({"empirical b probs": wandb.Image(fig)}, step=epoch)
                plt.close()
            log_corpus[dataset_name + "_bpe_dict_param_fold_" + str(fold_idx)] = (
                bpe_dict_param[1]
            )
    ...
    if wandb_realtime:
        ...
        if kwargs["visualise"]:
            data_format = (
                "nx"
                if agent.env.isomorphism_module.__class__.__name__ == "ExactIsomoprhism"
                else "csr"
            )
            visualise_subgraphs(
                agent.env.dictionary,
        ...
        fig = plt.figure()
        x_a_fractional = dictionary_probs_model.membership_vars(
            len(agent.env.dictionary)
        ).tolist()[0 : len(agent.env.dictionary)]
        plt.bar(list(range(len(agent.env.dictionary))), x_a_fractional)
        wandb.log(
            {"stats/membership fractional variables": wandb.Image(fig)}, step=epoch
        )
        plt.close()
        fig = plt.figure()
        plt.bar(list(range(len(b_probs))), b_probs.tolist())
        wandb.log({"stats/b probs": wandb.Image(fig)}, step=epoch)
        plt.close()
        if delta_prob is not None:
            fig = plt.figure()
            plt.bar(
                [0, 1], [1 - delta_prob.sigmoid().item(), delta_prob.sigmoid().item()]
            )
            wandb.log(
                {"stats/learned subgraph types probs": wandb.Image(fig)}, step=epoch
            )
            plt.close()
        fig = plt.figure()
        plt.bar(
            list(range(len(agent.env.dictionary))),
            atom_probs[: len(agent.env.dictionary)].tolist(),
        )
        wandb.log({"stats/learned atom probabilities": wandb.Image(fig)}, step=epoch)
        plt.close()
        if cut_size_probs is not None:
            fig = plt.figure()
            plt.bar(list(range(len(cut_size_probs))), cut_size_probs.tolist())
            wandb.log({"stats/learned cut size probs": wandb.Image(fig)}, step=epoch)
            plt.close()
        fig = plt.figure()
        plt.bar(
            list(range(len(agent.env.dictionary))),
        ...
        wandb.log({"stats/empirical atom frequencies": wandb.Image(fig)}, step=epoch)
        plt.close()
        wandb.log(log_corpus, step=epoch)
        with open(os.path.join(wandb.run.dir, "selected_atom_inds.txt"), "a+") as f:
            sorted_probs_inds = torch.argsort(
                atom_probs[0 : len(agent.env.dictionary)], descending=True
            )
...
def logger(compression_folds, dataset_names, perf_opt, wandb_flag, wandb_realtime):
        ...
        for epoch in range(len(datasets_compression_mean[0])):
            ...
            for i, dataset_name in enumerate(dataset_names):
                for fold_idx in range(len(datasets_compression_folds[i])):
                    log_corpus[dataset_name + "_compression_fold_" + str(fold_idx)] = (
                        datasets_compression_folds[i][fold_idx, epoch]
                    )
                # log epoch score means across folds
                log_corpus[dataset_name + "_compression_mean"] = (
                    datasets_compression_mean[i][epoch],
                )
                log_corpus[dataset_name + "_compression_std"] = (
                    datasets_compression_std[i][epoch],
                )
            wandb.log(log_corpus, step=epoch)
    ...
    for i, dataset_name in enumerate(dataset_names):
        if wandb_flag:
            ...
            wandb.run.summary["best_" + dataset_name + "_mean"] = (
                datasets_compression_mean[i][best_index]
            )
            wandb.run.summary["best_" + dataset_name + "_std"] = (
                datasets_compression_std[i][best_index]
            )
            wandb.run.summary["last_" + dataset_name + "_mean"] = (
                datasets_compression_mean[i][-1]
            )
            wandb.run.summary["last_" + dataset_name + "_std"] = (
                datasets_compression_std[i][-1]
            )
        print(
            "Best {} mean: {:.4f} +/- {:.4f}".format(
                dataset_name,
                datasets_compression_mean[i][best_index],
                datasets_compression_std[i][best_index],
            )
